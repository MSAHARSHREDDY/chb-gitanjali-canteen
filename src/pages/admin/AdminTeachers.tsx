...
export function AdminTeachers() {
  ...
  const [roleFilter, setRoleFilter] = useState<'all' | 'parent' | 'teacher' | 'admin'>('teacher');
  ...
  return (
    <div className="space-y-6">
      <div className="bg-gradient-to-r from-[#0d1530] to-[#080d22] border border-white/10 rounded-2xl p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 shadow-xl">
        <div>
          <span className="text-blue-400 text-xs font-mono tracking-widest uppercase mb-1 block">Faculty Administration Panel</span>
          <h3 className="text-3xl font-bold font-serif text-white flex items-center gap-3">
            <Users className="w-8 h-8 text-blue-500" />
            Manage Teachers
          </h3>
          <p className="text-slate-400 text-sm mt-1">Audit active teachers, track subscription menu statuses, and remove accounts.</p>
        </div>
        <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl px-4 py-2 flex items-center gap-3">
          <Users className="w-5 h-5 text-blue-400" />
          <div>
            <span className="block text-[10px] font-mono text-gray-500 uppercase">Teachers Registered</span>
            <span className="text-sm font-bold text-white font-mono">{roleCounts.teacher} teachers</span>
          </div>
        </div>
      </div>
