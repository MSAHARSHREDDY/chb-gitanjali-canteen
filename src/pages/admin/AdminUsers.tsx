  ...
  return (
    <div className="space-y-6">
      <div className="bg-gradient-to-r from-[#0d1530] to-[#080d22] border border-white/10 rounded-2xl p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 shadow-xl">
        <div>
          <span className="text-blue-400 text-xs font-mono tracking-widest uppercase mb-1 block">System Users Panel</span>
          <h3 className="text-3xl font-bold font-serif text-white flex items-center gap-3">
            <Users className="w-8 h-8 text-blue-500" />
            Registered Users
          </h3>
          <p className="text-slate-400 text-sm mt-1">Manage parent and teacher accounts, track active plans, and administrate access.</p>
        </div>
        <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl px-4 py-2 flex items-center gap-3">
          <Users className="w-5 h-5 text-blue-400" />
          <div>
            <span className="block text-[10px] font-mono text-gray-500 uppercase">Users Registered</span>
            <span className="text-sm font-bold text-white font-mono">{users.length} accounts</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex overflow-x-auto gap-2 pb-2 scrollbar-hide">
        {[
          ...
          >
            {tab.label} ({roleCounts[tab.id as keyof typeof roleCounts]})
          </button>
        ))}
      </div>
